...
appUI.populateCountriesAll = function() {
	localStore.getData("SELECT *", null, null, function(localData) {			
		if (localData.length>0) {
			...
			var regionNames = ["Asia Pacific", "North America"];
			
			
			var len = regionNames.length;
			...
			//for (i=0; i<len; i++) {
				var regionKey = regionNames[i]; //localData[i].region;
				alert(i + ' ' + regionNames[i]);
				...
				// section display for REGIONS
				//alert(localData[i].region);
...
appUI.launchFirstScreen = function( countryCode ) {	
	localStore.getData("SELECT *", null, null, 
		function(localData) {
			...
			$("#allCountries ul li:first-child").addClass("hidden");
			alert("hello");
		}
	);
}
